"""
Simple Demo - Fuzzy Movie Recommendation System
Minimal version using only standard library + basic packages
"""
    ...
    def predict_movie(self, user_preferences, movie_data):
        ...
        for genre in movie_genres:
            if genre in user_preferences:
                pref_score = user_preferences[genre]
        ...
        return {
            ...
            'explanation': f"Genre match for {', '.join(movie_genres)} with preferences"
        }
...
def run_server(port=5000):
    server_address = ('', port)
    httpd = HTTPServer(server_address, MovieRecommendationHandler)
    print(f"🚀 Simple Movie Recommendation Server running on http://localhost:{port}")
    print(f"📊 Loaded {len(SAMPLE_MOVIES)} sample movies")
    print(f"🎯 Available genres: {', '.join(recommender.genres)}")
    print(f"💡 Try: http://localhost:{port}/api/health")
